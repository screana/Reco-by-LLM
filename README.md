...
ユーザ履歴9件のタイトルと属性をプロンプトに渡し、LLMに「次に検索しそうなクエリ」
を生成させる。生成クエリをベクトル検索へ投げて関連記事を取得し、必要に応じて
別視点のクエリも複数生成して候補を合流させる。最終的には類似度スコアでソートし、
上位30件程度を推薦候補として返す。LLMが生成したクエリやトピック説明は、そのまま
推薦理由の説明に使えるのが強み。リアルタイム性はクエリ生成回数を抑えたりキャッシュで
補う方針。実運用では軽量なLLMを使い応答時間を抑える。
